...
func TestGenerateCas(t *testing.T) {
	...
	gas := round((val * (getUnClaimedGas(1992850, 2001227) + 2) / totalCAM), 8)
...
func getBlocks(start int64, end int64) ([]*camdb.Block, error) {
...
func unspent(address string, asset string) ([]*rpc.UTXO, error) {
	...
	for _, t := range tutxos {
		utxos = append(utxos, &rpc.UTXO{
			...
			CreateTime: strconv.FormatInt(t.CreateTime,10),
			Block:      t.CreateBlock,
