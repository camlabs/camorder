...
func blocksFee(blocks []*camdb.Block, start, end int64) (float64, int64) {
	if len(blocks) == 0 {
		return 0, 0
	}
...
// CalcUnclaimedGas .
func CalcUnclaimedGas(unclaimed []*rpc.UTXO, blocks []*camdb.Block) (unavailable, available float64, err error) {
...
func calcUnclaimedGas(unclaimed []*rpc.UTXO, blocks []*camdb.Block) (unavailable, available float64, err error) {
	...
	for _, utxo := range unclaimed {
		...
		camFloat := float64(totalCAM)

		gasFixed8 := tx.MakeFixed8(val * gas / camFloat)
